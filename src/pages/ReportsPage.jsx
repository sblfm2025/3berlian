    ...
    <div className="max-w-7xl mx-auto space-y-3 sm:space-y-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div className="hidden md:block">
          ...
          <h2 className="mt-1 text-lg font-bold text-slate-900 sm:text-2xl sm:font-black">Rekap operasional bulan ini</h2>
        ...
        <div className="flex flex-nowrap items-center gap-2 overflow-x-auto pb-1 md:flex-wrap md:overflow-visible md:pb-0">
          <label className="flex shrink-0 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 sm:rounded-[18px] sm:px-4 sm:py-3 sm:text-sm">
            ...
            <input
              ...
              className="border-none bg-transparent text-xs font-bold text-slate-800 focus:outline-none sm:text-sm"
            />
          </label>
          <select
            value={statusFilter}
          ...
          <button
            ...
            className="shrink-0 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 sm:rounded-[18px] sm:px-4 sm:py-3 sm:text-sm"
          >
            Reset filter
          </button>
        </div>
      ...
      <div className={`rounded-2xl border border-slate-200 bg-white px-3 py-2.5 shadow-sm flex flex-wrap items-center justify-between gap-2 sm:rounded-[20px] sm:px-4 sm:py-3 sm:gap-3 ${searchTerm ? 'hidden md:flex' : ''}`}>
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-400 sm:text-[11px] sm:tracking-[0.2em]">Ringkasan filter aktif</p>
          <p className="mt-1.5 text-sm font-semibold text-slate-900 sm:mt-2 sm:font-black">{selectedMonth} - {statusFilter} - {paymentMethodFilter}</p>
        </div>
      ...
      <div className={`grid grid-cols-2 gap-2 sm:gap-3 md:grid-cols-2 xl:grid-cols-5 ${searchTerm ? 'hidden md:grid' : ''}`}>
        {summaryCards.map((card, index) => (
          <div key={card.title} className={`rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:rounded-[20px] sm:p-5 ${index === 2 ? 'col-span-2 md:col-span-1' : ''} ${index > 2 ? 'hidden md:block' : ''}`}>
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500 sm:text-xs sm:tracking-[0.2em]">{card.title}</p>
            <p className="mt-2 break-words text-lg font-bold text-slate-900 sm:mt-3 sm:text-2xl sm:font-black">{card.value}</p>
            <p className="mt-1.5 text-xs text-slate-500 sm:mt-2 sm:text-sm">{card.description}</p>
          </div>
        ))}
      </div>
      ...
      {hasTransactions && (
      <div className={`grid gap-3 sm:gap-4 xl:grid-cols-[1.1fr_0.9fr] ${searchTerm ? 'hidden md:grid' : ''}`}>
        <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:rounded-[24px] sm:p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-slate-500">Komposisi pembayaran</p>
              <h3 className="mt-1 text-base font-bold text-slate-900 sm:text-lg sm:font-black">Metode transaksi</h3>
            </div>
        ...
        <div className="grid gap-3 sm:gap-4">
          <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:rounded-[24px] sm:p-5">
            <div>
              ...
              <h3 className="mt-1 text-base font-bold text-slate-900 sm:text-lg sm:font-black">Pelanggan teratas periode ini</h3>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {topCustomerList.length === 0 ? (
                ...
                <div key={name} className="flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-3 py-2.5 sm:rounded-[18px] sm:px-4 sm:py-3">
                  <div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:rounded-[24px] sm:p-5">
            <div>
              ...
              <h3 className="mt-1 text-base font-bold text-slate-900 sm:text-lg sm:font-black">Prioritas tindak lanjut</h3>
            </div>
            ...
            <div className="mt-4 space-y-3">
              {overdueTransactions.length === 0 ? (
                ...
                <div key={tx.id} className="rounded-2xl bg-red-50 border border-red-100 px-3 py-2.5 sm:rounded-[18px] sm:px-4 sm:py-3">
      ...
      {hasTransactions && (
      <div className={`rounded-2xl border border-slate-200 bg-white p-3 shadow-sm sm:rounded-[24px] sm:p-5 ${searchTerm ? 'hidden md:block' : ''}`}>
        <div>
          <p className="text-sm font-semibold text-slate-500">Produk paling laris</p>
          <h3 className="mt-1 text-base font-bold text-slate-900 sm:text-lg sm:font-black">Kostum terlaris per periode</h3>
        </div>
      ...
      <div className={`flex flex-wrap gap-2 sm:gap-3 ${searchTerm ? 'hidden md:flex' : ''}`}>
        <button onClick={handleExportExcel} disabled={isExporting !== ''} className="flex-1 md:flex-none flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-3 py-2.5 text-xs font-bold text-white shadow-sm sm:rounded-[18px] sm:px-5 sm:py-3 sm:text-sm sm:font-black">
          {isExporting === 'excel' ? <><Cloud size={18} className="animate-pulse" /> Proses...</> : <><FileSpreadsheet size={18} /> Unduh Excel</>}
        </button>
        <button onClick={handleExportPDF} disabled={isExporting !== ''} className="flex-1 md:flex-none flex items-center justify-center gap-2 rounded-xl bg-rose-600 px-3 py-2.5 text-xs font-bold text-white shadow-sm sm:rounded-[18px] sm:px-5 sm:py-3 sm:text-sm sm:font-black">
            ...
            <div key={t.id} className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">{formatDate(t.rentDate)}</p>
              ...
              <div className="mt-2.5 rounded-xl bg-slate-50 px-3 py-2 text-xs">
                ...
                <p className="mt-1 font-bold text-slate-900">{t.paymentMethod || 'Tunai'}</p>
              </div>
              <div className="mt-3 flex justify-between items-center border-t border-slate-100 pt-3">
                <div>
                  <p className="text-xs text-slate-500">Total</p>
                  <p className="text-sm font-bold text-slate-900">{formatCurrency(grandTotal)}</p>
                </div>
