    ...
    <div className="max-w-7xl mx-auto flex flex-col gap-3">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 lg:min-w-[320px]">
            <div className="rounded-[20px] bg-white/10 border border-white/20 p-3 backdrop-blur-sm">
              <p className="text-[10px] uppercase tracking-[0.12em] text-white/80 sm:text-[11px] sm:tracking-[0.18em]">Item keranjang</p>
              <p className="mt-2 text-base font-bold sm:text-xl">{totalItems}</p>
            </div>
            <div className="rounded-[20px] bg-white/10 border border-white/20 p-3 backdrop-blur-sm">
              <p className="text-[10px] uppercase tracking-[0.12em] text-white/80 sm:text-[11px] sm:tracking-[0.18em]">Produk ready</p>
              <p className="mt-2 text-base font-bold sm:text-xl">{products.filter(product => product.stock > 0).length}</p>
            </div>
            <div className="rounded-[20px] bg-white/10 border border-white/20 p-3 backdrop-blur-sm sm:col-span-1 col-span-2">
              <p className="text-[10px] uppercase tracking-[0.12em] text-white/80 sm:text-[11px] sm:tracking-[0.18em]">Stok menipis</p>
              <p className="mt-2 text-base font-bold sm:text-xl">{lowStockCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-[24px] border border-slate-200 bg-white p-4.5 shadow-sm space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <form onSubmit={handleQrScanSubmit} className="flex gap-2 max-w-md w-full shrink-0">
            ...
            <button
              type="submit"
              className="rounded-xl bg-blue-800 hover:bg-blue-900 text-white px-3 py-2.5 text-xs font-semibold shadow-sm flex items-center gap-1 transition sm:rounded-2xl sm:px-4"
            >
              Proses
            </button>
          </form>
      ...
      <div className="hidden gap-3 sm:grid-cols-2 md:grid xl:grid-cols-4">
        <div className="rounded-[24px] border border-slate-200 bg-white px-4 py-3 shadow-sm">
          ...
          <p className="mt-2 text-sm font-bold text-slate-900">{customerNameInput || 'Belum diisi'}</p>
        </div>
        <div className="rounded-[24px] border border-slate-200 bg-white px-4 py-3 shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-[11px] sm:tracking-[0.18em]">Tanggal kembali</p>
          <p className="mt-2 text-sm font-bold text-slate-900">{formatDate(returnDateInput || defaultDateStr)}</p>
        </div>
        <div className="rounded-[24px] border border-slate-200 bg-white px-4 py-3 shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-[11px] sm:tracking-[0.18em]">Pembayaran</p>
          <p className="mt-2 text-sm font-bold text-slate-900">{paymentMethod}</p>
        </div>
        <div className="rounded-[24px] border border-slate-200 bg-white px-4 py-3 shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-[11px] sm:tracking-[0.18em]">Deposit</p>
          <p className="mt-2 text-sm font-bold text-slate-900">{depositAmount > 0 ? formatCurrency(depositAmount) : 'Tidak ada'}</p>
        </div>
      </div>
      ...
      <div className="hidden gap-3 md:grid xl:grid-cols-[1fr_1fr]">
        <div className="rounded-[24px] border border-amber-200 bg-amber-50 px-4 py-3 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-amber-700 sm:text-[11px] sm:tracking-[0.18em]">Stok menipis</p>
              <p className="mt-2 text-sm font-bold text-slate-900">{lowStockCount} produk butuh perhatian</p>
            </div>
            <button
              type="button"
        ...
        <div className="rounded-[24px] border border-blue-200 bg-blue-50 px-4 py-3 shadow-sm">
          <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-blue-700 sm:text-[11px] sm:tracking-[0.18em]">Ringkasan keranjang</p>
      ...
      <div className="hidden rounded-[20px] border border-slate-200 bg-white p-4 shadow-sm md:block">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500 sm:text-[11px] sm:tracking-[0.18em]">Stok per kategori</p>
          ...
          {categories.filter(category => category !== 'Semua').map(category => {
            ...
            return (
              <div key={category} className="rounded-[20px] bg-slate-50 px-4 py-3 border border-slate-100">
                <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 sm:text-[11px] sm:tracking-[0.18em]">{category}</p>
                <p className="mt-2 text-base font-bold text-slate-900 sm:text-lg">{available} tersedia</p>
