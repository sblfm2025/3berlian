  ...
  return (
    ...
    >
      <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-xl bg-slate-100 sm:h-36 sm:w-full sm:rounded-none" onClick={() => !isSelected && updateCartQty(product, 1)}>
        ...
        <div className={`absolute bottom-1 left-1 rounded-full px-1.5 py-0.5 text-[8px] font-bold sm:bottom-3 sm:left-3 sm:px-3 sm:py-1 sm:text-[11px] ${stockBadgeClass}`}>
          {stockLabel}
        </div>
        {isSelected && (
      ...
      <div className="min-w-0 flex-1 sm:p-4">
        <div className="flex flex-wrap gap-1.5 sm:gap-2">
          <span className="max-w-full break-words rounded-full bg-blue-50 px-2 py-0.5 text-[9px] font-bold text-blue-700 sm:text-[10px] sm:px-2.5 sm:py-1">
            {product.category || 'Lainnya'}
          </span>
          <span className="max-w-full break-words rounded-full bg-slate-100 px-2 py-0.5 text-[9px] font-bold text-slate-600 sm:text-[10px] sm:px-2.5 sm:py-1">
            {product.size || 'All Size'}
          </span>
        </div>
        <h4 className="mt-1 line-clamp-2 break-words text-[13px] font-semibold leading-snug text-slate-900 sm:mt-3 sm:text-sm sm:font-bold">{product.name}</h4>
        <p className={`mt-0.5 text-[9px] font-bold uppercase tracking-[0.08em] sm:mt-2 sm:text-[11px] sm:tracking-[0.18em] ${stockStatus === 'habis' ? 'text-red-600' : stockStatus === 'menipis' ? 'text-amber-700' : 'text-emerald-700'}`}>
          {statusText}
        </p>
        <div className="mt-1.5 flex items-end justify-between gap-2 sm:mt-3 sm:flex-row sm:gap-3">
          ...
          {isSelected ? (
            <div className="flex shrink-0 items-center gap-1.5 rounded-[14px] border border-blue-100 bg-blue-50 px-1.5 py-1 sm:gap-2 sm:rounded-[18px] sm:px-2 sm:py-1.5">
              ...
              <button type="button" onClick={() => updateCartQty(product, 1)} className="min-h-10 min-w-10 rounded-lg bg-blue-700 p-1.5 text-white shadow-sm hover:bg-blue-800 sm:rounded-xl sm:p-2">
