  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1fr_1.1fr]">
      <div className="rounded-2xl bg-slate-50 p-3 border border-slate-100 sm:rounded-[22px] sm:p-4">
        ...
        <details className="mt-5 rounded-2xl border border-slate-200 bg-white p-3">
          <summary className="cursor-pointer text-xs font-bold text-slate-700">Atur denda manual / override</summary>
          <p className="mt-2 text-xs font-semibold text-amber-700">Gunakan hanya jika ada keputusan admin.</p>
          <div className="mt-3 space-y-3">
            <label className="block">
              <span className="block text-[11px] font-semibold text-slate-500">Nominal Denda (Rp)</span>
              <input
                type="number"
                min="0"
                placeholder={lateFee}
        ...
        <div className="mt-3 space-y-2 text-sm">
          ...
          {lateFee > 0 && (
            <div className="rounded-2xl bg-red-50 px-3 py-2 text-xs font-semibold text-red-700">
              {lateDays} hari x {formatCurrency(dailyFine)} = {formatCurrency(lateFee)}
            </div>
          )}
          <div className="flex justify-between text-slate-600">
